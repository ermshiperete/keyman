...
class DiagReport:
    ...
    def _get_freedesktop_os_release_info(self, os_info):
        ...
        os_info['name'] = os_release.get('NAME', '')
        os_info['pretty_name'] = os_release.get('PRETTY_NAME', '')
        os_info['version'] = os_release.get('VERSION', '')
        os_info['version_id'] = os_release.get('VERSION_ID', '')
        os_info['version_codename'] = os_release.get('VERSION_CODENAME', '')
        os_info['id'] = os_release.get('ID', '')
    ...
    def _get_input_method(self):
        ...
        return {
            ...
            'input_method': os.environ.get('INPUT_METHOD', '')
        }
    ...
    def _get_browser_versions(self):
        """Get versions of Firefox, Chromium, Chrome installed via apt, snap, flatpak."""
        return {
    ...
    def _get_report(self):  # sourcery skip: low-code-quality
        ...
        lines.append(f"  IBus version: {ibus_version or 'Not available'}")
        ...
        if 'pretty_name' in os_info and os_info['pretty_name']:
            ...
            lines.append(f"  OS: {os_info['description']}")
        else:
        ...
        if browsers['apt']:
            ...
            for name, version in browsers['apt'].items():
                lines.append(f"    - {name}: {version}")
        if browsers['snap']:
            has_browsers = True
            lines.append("  Snap packages:")
            for name, version in browsers['snap'].items():
                lines.append(f"    - {name}: {version}")
        if browsers['flatpak']:
            has_browsers = True
            lines.append("  Flatpak packages:")
            for name, version in browsers['flatpak'].items():
                lines.append(f"    - {name}: {version}")
        if not has_browsers:
