  ...
  private loadSerializedOptions(): km_core_option_item[] {
    ...
    for (const defaultOption of defaultOptions) {
      if (defaultOption.scope == KM_CORE_OPTION_SCOPE.OPT_KEYBOARD) {
        const savedValue = this.variableStoreSerializer.loadStore(toPrefixedKeyboardId(this.KmxKeyboard.id), defaultOption.key);
        const value = savedValue || defaultOption.value;
